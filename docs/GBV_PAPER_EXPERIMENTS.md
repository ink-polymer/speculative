...
这套代码以 K=3、L=15 的 GBV 为主方法，评测 Qwen3-4B 和 Qwen3-8B 两组 Target 与各自配套的 DFlash-b16 扩散草稿模型。当前采用 12 配置精简方案：4B 做五方法主实验及 K=2、K=4 路径数消融，8B 做五方法主实验。两个模型均保留上机前的小规模贪心正确性检查。主实验、消融、评分和汇总共用 `src/gbv_experiments/`，入口是 `scripts/gbv_paper.py`。尚未产生 H200 正式实验结果；本机测试不能作为论文速度结果。
...
- 块宽为 16：1 个已知 anchor + 15 个未来位置。`length=L` 不含 anchor；当前所有正式配置固定 L=15，不调度长度消融。
- 所有正式配置的 Target 温度为 T=1；GBV、BV 与 DDTree 的草稿概率温度也为 1。DFlash 匹配基线始终以 argmax 构造草稿。T=0 只用于少量上机前正确性检查，不生成整套评测表。
...
`dflash_match` 按 [DDTree 中的 DFlash 实现](https://github.com/liranringel/ddtree/blob/c96427a185677bf4133ed865dd1626a5041aef9b/dflash.py) 构造贪心草稿，依次接受与 Target 采样 token 匹配的最长前缀，并提交首次不匹配处的 Target token。标准 p/q 拒绝采样并不等于这个规则；其 `single_token_rejection` 实现仅作为可选兼容功能保留，不包含在当前 12 配置中。
...
DDTree 预算由 60 调整为 45 个非根节点，对应 GBV 的 3×15 个候选位置上限；根 anchor 另计。前缀共享后 GBV 实际节点可能少于 45，且两种方法构树与校正成本不同，因此该对照不是严格等实际 FLOPs 或等耗时。报告同时给出实际树节点、接受长度和 forward 次数；此配置也不代表官方 DDTree 经预算搜索得到的最优性能。当前结论应限定为该框架与预算下的对比；若要声称超过 DDTree 的最佳速度，需要另做预算对照。[官方主表](https://arxiv.org/html/2604.12989v1#S5.S2) 在多个预算中选择最佳结果。

当前只保留一组核心消融：在 4B 上比较候选数 K=1、2、3、4，固定 L=15、T=1。该对照回答增加候选的接受收益能否抵消开销，以及 K=3 与相邻候选数的取舍，不预设 K=3 一定最优。

K=1 的 GBV 在分布构造上退化为单路径 BV，直接复用 `dflash_bv`；K=3 复用 `gbv`。因此只新增 `gbv_k2`、`gbv_k4` 两个配置。全部结果按预定网格报告，不根据测试分数挑选有利设置。

| 模型 | 正式配置 | 数量 |
|---|---|---:|
| Qwen3-4B | target_t1、dflash_match、dflash_bv、gbv、ddtree、gbv_k2、gbv_k4 | 7 |
| Qwen3-8B | target_t1、dflash_match、dflash_bv、gbv、ddtree | 5 |

本轮不调度验证长度、单路径 token/BV、前缀共享开关或全评测集 T=0 对照；不调度双向注意力因果化、Target 特征置零、Draft 缓存开关、概率精度切换、K=8 或额外中间温度。所有正式配置仍开启前缀共享；不单独声称该优化带来了多少加速。源码中的旧开关仅供兼容和内部回归检查，不增加正式实验任务。

小规模真实 checkpoint 的贪心一致性、树分支和缓存回退检查继续作为上机门槛保留；数据审计、参考答案/评分器自检、质量评分和置信区间计算也继续执行。它们不是新增的整套模型评测配置，正确性检查的少量生成不计入下表。

| 累计阶段 | 4B 配置数 | 8B 配置数 | 数据集×配置×种子任务数 | 题目/完整对话记录 | 回答生成次数 |
|---|---:|---:|---:|---:|---:|
| gbv-first | 1 | 1 | 42 | 4,716 | 5,196 |
| main | 5 | 5 | 210 | 23,580 | 25,980 |
| complete | 7 | 5 | 252 | 28,296 | 31,176 |
...
# 同一输出目录：补齐 4B 的 K=2、K=4，8B 不再新增配置。
...
GBV 优先阶段尚无全量 AR 对照，报告只呈现吞吐、接受长度与可评分任务的质量；加速比和相对 AR 的质量差值留空，整体报告标记未完成。`main` 阶段补齐基线后即可查看两模型完整 T=1 主表；此时 8B 的全部正式配置完成，4B 仍需在 `complete` 阶段补齐 K=2、K=4。只有最终阶段要求所有配置生成和评分齐全，并绘制主实验和消融图。
...
首次运行即在每个模型的 manifest 中固定该模型的完整矩阵。阶段只是调度过滤条件，不改变实验身份，因此可安全续跑。代码、环境、模型、完整配置或数据清单改变时拒绝复用旧结果；已有旧 20 配置套件或更早实验结果时须使用新输出目录，不能直接续写不同矩阵。不同阶段没有跨阶段随机交错，正式比较应维持同一专用 GPU、功率/时钟设置及后台负载；按题交错仅作用于同阶段尚未完成的方法，不能据此声称完全消除了跨阶段时间漂移。
...
全源划分备选使用 `configs/gbv_paper_full.json`，并为所有准备、审计、预检、生成、评分步骤指定独立的数据和输出目录。它仅包含 4B 的 7 配置：每个配置/种子 3,648 题或对话、3,728 次回答，三种子合计 76,608 条记录、78,288 次回答，不属于默认两模型套件。固定子集与全源划分须分别报告。
...
- 本轮 T=0 一致性仅记录于小规模 GPU 预检文件。预检同时保存 `greedy_exact_passed` 和总门禁结果：严格逐 token 相同时前者为真；若 BF16/SDPA 的树批量计算与单 token 缓存计算只在 top-1 近并列处翻转，只有在 AR 与候选重复运行均稳定、实测最大 logits 误差不超过 0.5、且两边 top-1 间隔均不大于两倍实测误差时，才可标记为 `bounded_bf16_near_tie` 并通过 T=1 正式运行门禁。该记录不构成 T=0 有限精度逐 token 无损声明。非 BF16、结构/KV 错误、宽间隔翻转、大误差或重复不稳定仍硬失败。正式 T=1 评测不要求相同 seed 逐 token 一致，因为不同算法消耗随机数的顺序不同。
...
实验性 `tree_gbv` 方法先用 best-first Cartesian 搜索保留 DFlash 乘积分布中质量最高的完整路径，将其归一化为有限自回归 Tree 提议分布，再从该分布独立抽取 K 条路径。GBV 的字典序使用每条候选实际的前缀条件概率；选中后按同一 Tree 提议分布逐前缀重加权，最后调用批量 BV。`tree_budget` 在这里表示有限提议分布的叶子数，不是 DDTree 的非根节点预算；实际 Target 验证节点数另外记录。该方法没有加入正式配置矩阵，必须显式构造 `Variant(method="tree_gbv", ...)` 才会运行。

Tree 提议并不破坏校正链：测试在非独立、前缀相关的小词表提议分布上枚举全部候选组合，逐项核对 GBV 选中分布；随后枚举参考与批量 BV 的全部采样分支，核对最终联合输出仍等于 Target 自回归分布。这里的“无损”指采样律经校正保持不变；FP64 机器计算仍受舍入影响。2026-09-05 的隔离 GH200 筛选固定 Target/Draft 温度均为 1.0，最快复测配置为 K=2、L=7、8 个提议叶子：59.481 tok/s（2.498× AR），原同温 GBV 为 59.048 tok/s（2.480×），DDTree 为 87.871 tok/s（3.690×）。因此当前实现验证了组合的正确性和小幅 GBV 收益，但没有超过 DDTree；不得使用单提示粗筛峰值代替多提示复测结论。

后续完整有限树版本解析边缘化 K 次 IID 候选抽样，并在 119 个邻近配置中复验。优化前最佳为 K=10、L=14、6 个叶子：75.694 tok/s（3.207× 同任务 AR，DDTree 的 88.31%）；平均每轮接受 3.807、提交 4.618 个 token，DDTree 分别为 5.204、6.019。框架数据流优化把选路阶段需要的 Target 张量从 `leaves × (L+1) × vocab` 延迟为 `leaves × L` 个选中词元概率，再只为实际选中的路径提取 `L+1` 个完整行；BV 的 proposal 也从 `L × vocab` 改为每叶一列、用 `scatter_add_` 直接扣减。以该配置和 151,936 词表的 FP64 概率为例，选路前的 Target 行物化从约 104.3 MiB 降到约 17.4 MiB，proposal 从约 16.2 MiB 降到 1,344 bytes（不计张量固定开销）。

同一 GH200、同一进程、5 提示 × 2 seeds 的交错 A/B（作业 2060876）中，稀疏版本相对保留的稠密参考版本吞吐提高 6.74%（64.758 vs 60.668 tok/s），`select_and_correct` CUDA 时间减少 39.92%（855.93 vs 1424.69 ms）；两者轮数、接受长度、提交长度和树节点数逐项相同。稀疏版本在该次运行仍只有 DDTree 吞吐的 78.97%，主要差距已不是这份张量物化，而是提交长度 4.618 对 6.019。不同 GH200 节点的绝对吞吐波动明显，前后效果只采用同进程 A/B，不用跨节点的绝对 tok/s 归因。

原先直接计算两项近似相等的幂之差可能在长前缀中产生全零概率行。新内核在缩放后用非负多项式计算这个差商，并逐层更新相对质量，主配置用 FP64。遇到不合法概率报错，不静默替换为其他分布。这个实现减少已知的消减误差，不等于机器浮点与实数严格相等。

本地测试包括小词表下的有理数全枚举、枚举实际 BV 采样分支验证最终联合输出分布、深前缀数值退化回归，以及真实小型 Qwen3/DFlash 的树/顺序 logits、特征、缓存、EOS 和长度边界校验。数据测试覆盖 AIME 原始题号、LiveCodeBench 六文件加载与私有测试解码/执行、MT-Bench 第二轮历史、分轮 token 计数、完整对话续跑和裁判导出/导入。选题测试对照实际 Hugging Face shuffle，核查原始题号与顺序、源条数、全量/子集目录隔离、入选题完整测试用例及固定子集从准备到评分汇总的流程；缺题、重复题号或替换题号不能生成完整报告。这些使用本地构造的数据，不替代真实数据准备与审计。H200 上另执行真正 checkpoint 的 greedy、树分支及缓存回退校验；若出现逐 token 差异，报告保留完整 token、重复运行、top-1 间隔和树/顺序 logits 误差证据。新增测试覆盖 DFlash 匹配验证的联合输出分布、T=1 贪心草稿规则、Tree+GBV 前缀条件分布及端到端 BV 联合输出律、稀疏/稠密条件分布一致性、8B 固定配置的 meta 构造、BF16 近并列分类器的放行与失败边界，以及先 GBV 后补齐的断点续跑。远端隔离环境最终 166 项全部通过。CPU 测试通过不能替代 H200/BF16 校验；结构或无法量化解释的数值门禁不通过时不得生成正式速度结果。
...
实验性的 `tree_gbv_full` 同样不作算法原创性声明。它组合了 GBV 的有序路径选择与校正、DDTree 的扩散草稿树/祖先 mask，以及有限离散分布上 IID 最大值的标准次序统计。当前新增的延迟展开（选中叶子后才提取完整 Target 行）和稀疏 proposal `scatter_add_` 校正是本仓库独立编写的工程数据流优化；截至 2026-09-05 的关键词与相关工作检索未找到逐项相同的公开实现，但这不足以证明文献或专利意义上的新颖性，论文不得据此使用“首创”等措辞。按轮动态选择树预算也不作为本方法贡献，因为 [CaDDTree](https://arxiv.org/abs/2606.01813) 已公开提出按当前分布与验证成本自适应预算。
