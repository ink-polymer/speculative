...
> 本分支的 65,280-call 矩阵只用于树状块新服务器的全新运行。不得用它
> 续跑或覆盖此前基于 `16c0e91` 启动过的 59,904-call、不含树状块的任务。
> 新运行必须使用新输出目录并重做全部输入/源码完整性门禁。
...
如果先只在 H20 上执行 Qwen3-4B 的 T=1，该登记范围是 816 条 source/conversation、
9,792 条方法结果记录和 10,752 个实际生成轮次，范围名称固定为
**“Qwen3-4B / T=1 / H20 登记子矩阵”**。它通过后最多只能记为
`submatrix_complete=true`；`formal_complete`、`whole_formal_matrix_complete`、
`publication_claim_eligible` 和 `tree_block_superiority_claim_eligible` 仍必须为 `false`。
这些标志只能由完整 4B/8B、T=0/T=1 的 65,280-call 矩阵及全部门禁决定。
...
资格试跑永久不能支持论文结论。只有新服务器完成全部 4B/8B、八数据集、
