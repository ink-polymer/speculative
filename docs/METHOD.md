...
### 5.1 PPO-DDTree：以端到端时间为回报的主 RL 策略

此前 1720 条阶段实验中，LinUCB 约 79% 选择 100 节点、约 20% 选择 30 节点，几乎放弃
128 节点；它显著缩小了树，但平均提交长度同步下降，吞吐与手工 adaptive DDTree 基本持平。
因此新主策略改为离散 PPO，并把最大树预算提高到 256，动作集合扩展为：

```text
[30, 40, 50, 60, 70, 80, 90, 100, 112, 128, 144, 160, 192, 224, 256]
```

actor 对每个可用预算的 action features 独立打分后形成 categorical policy；features 包括预算
比例、log 预算比例、累计/边际 prefix mass、相对最大树的 mass 覆盖率、最高 prefix
probability 与 target KV 长度。critic 读取全部 action features 与 availability mask。训练时按
policy 采样，冻结评估时取最大 logit；每个 prompt 是一个 episode，每次 draft/verify round 是
一步，使用 GAE 和 clipped PPO objective 更新 actor/critic。

奖励直接对准真实吞吐：

```text
model_draft_ms      = max(0, draft_ms - tree_build_ms)
weighted_latency_ms = model_draft_ms + verify_ms
                      + tree_build_cost_weight * tree_build_ms
reward              = committed_tokens / weighted_latency_ms
```

默认 `tree_build_cost_weight=2`，使 Python best-first 枚举、mask 构造等随节点增长的成本受到
额外惩罚；由于先从 `draft_ms` 中减去 `tree_build_ms`，构树时间不会被无意重复计算。训练使用
与组合 2K 零重叠的 prompts，保存 actor、critic、optimizer、未满 rollout 和诊断信息；冻结
评估只读 checkpoint，不再采样或更新。

### 5.2 Contextual-bandit DDTree：历史复现基线

`tree_mode="ddtree_bandit"` 与原 JSON checkpoint 格式继续保留，用于复现 disjoint LinUCB
实验，但不再是默认 RL 路径。它仍采用单轮即时 tokens/ms 奖励，没有 PPO 的跨轮价值函数。

该策略没有导入或修改验证器；ancestor-only mask、最长 greedy path 和 KV compaction 完全复用。
