...
> 本分支实验包已改为 **原版非 RL Adaptive DDTree，官方 T=0 评测协议**，请从 [adaptivetree_paper/README.md](adaptivetree_paper/README.md) 进入。
> 十数据集采用官方 seed=0 抽样；包含介绍、证明、主实验、四项消融和测试，没有 RL 训练或 GBV 实验。以下历史内容不是该包的默认配置。
