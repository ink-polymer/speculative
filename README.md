...
- **Latency-aware DDTree**：一次枚举多个嵌套预算，用当前 proposal 的 prefix probability
  mass 与在线实测 GPU 延迟选择本轮树大小；不增加 draft forward，由
  `tree_mode="ddtree_adaptive"` 启用；
- **PPO-DDTree（主 RL 策略）**：actor 在 15 个嵌套树预算中选择节点数，critic + GAE
  处理同一 prompt 内的跨轮回报，奖励直接使用实际提交 token 与 draft/tree-build/verify
  时间；训练/冻结 checkpoint 分离，由 `tree_mode="ddtree_ppo"` 启用；
- **Contextual-bandit DDTree（历史基线）**：保留 disjoint LinUCB 实现与旧 checkpoint，
  便于复现此前近似持平的结果，由 `tree_mode="ddtree_bandit"` 启用；
...
## 草稿树拓扑：SpecBlock、DDTree 与 latency-aware DDTree
...
`ddtree_adaptive` 保持 DDTree 的节点排序不变。由于 best-first 产生的前 `b` 个节点对每个预算
`b` 都是合法嵌套树，它只在 `[30,45,60,80,100,128]` 中选择截断位置。选择目标是当前
proposal 预测的提交 token 数除以当前 GPU 在线测得的 round latency；各预算只做少量冷启动
观测，之后按 EWMA 更新。这替代了旧 engine 中依赖“上一轮接受长度”的滞后预算缩放。
benchmark 会额外记录 `tree_budget_histogram`、draft/verify/tree-build 分项耗时和平均树节点数，
用于确认策略选择与端到端收益是否一致。

运行 latency-aware DDTree：

```bash
CONFIG=configs/qwen3_4b_cuda_ddtree_adaptive.json \
OUTPUT=outputs/benchmark_ddtree_adaptive.jsonl \
...
```bash
# 默认使用与正式组合 2K 零重叠的 566 条训练集；训练后自动冻结并评估组合 2K。
bash scripts/run_rl_ppo_2k_remote.sh
```

PPO 的最大树预算为 256，动作空间为
`[30,40,50,60,70,80,90,100,112,128,144,160,192,224,256]`。奖励为
`committed_tokens / weighted_latency_ms`，其中构树时间从 draft 总时间中拆出并单独加权，
避免重复计时。PPO 仍只选择同一棵 best-first DDTree 的截断预算；`verification.py` 的
ancestor-only mask、greedy 路径接受和 KV cache 压缩没有任何改动。可变 verify shape 不能
与当前固定最大形状的 CUDA Graph verifier 叠加。
