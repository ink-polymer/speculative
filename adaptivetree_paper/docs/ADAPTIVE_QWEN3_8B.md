...
这是修正后非 RL、T=0 AdaptiveTree 的 8B 入口；完整矩阵原本已包含 8B，现在提供独立启动脚本和结果目录，不必同时运行 4B、30B。方法、证明、官方数据、历史控制与六项消融一致，不训练模型或策略。
...
- SDPA：官方 AR、DFlash、7 个 DDTree 预算，以及 8 个注册 Adaptive 方法（1 个主方法、1 个 legacy 历史对照、6 个消融/控制）；FA2：官方 AR、DFlash。Draft 始终 FA2。
- 正式 `adaptive` 候选预算为 30/45/60/80/100/128/160/192/256；包括 B=128、恢复旧成本归因、恢复探索、去接受校准、去延迟和预热后冻结六项消融，并保留 `adaptive_legacy` 历史控制。
- T=0，每回答最多 2,048 新 token。8B 完整子矩阵为 20 个进程组、21,888 次生成调用，另加预热。
