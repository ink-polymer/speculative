...
def main():
    ...
    for name in ("plan-tree-suite", "run-tree-suite"):
        p = sub.add_parser(name)
        p.add_argument("--suite", type=Path, default=ROOT / "configs/tree_bv_suite.json")
        p.add_argument("--phase", choices=["recycle-first", "main", "complete"], default="recycle-first")
        p.add_argument("--model-ids", nargs="+")
        p.add_argument("--output", type=Path, required=name == "run-tree-suite")
        if name == "run-tree-suite":
            p.add_argument("--data-dir", type=Path, default=ROOT / "datasets/gbv_paper_ddtree_counts")
            p.add_argument("--device", default="cuda:0")
            p.add_argument("--code-backend", choices=["docker", "process"], default="docker")
    ...
    elif args.command == "plan-tree-suite":
        from .tree_suite import plan_tree_suite
        plan = plan_tree_suite(args.suite, args.phase, args.model_ids)
        if args.output:
            write_json(args.output, plan)
        print(json.dumps(plan, ensure_ascii=False, indent=2))
