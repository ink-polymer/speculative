"""Discrete PPO policy for latency-aware DDTree node budgets.

The policy only selects a truncation point from one nested best-first DDTree.
Target verification, ancestor visibility and KV compaction are intentionally
outside this module and remain unchanged.
"""
...
class _Actor(nn.Module):
    """Score every available budget with one shared action network."""
...
class PPODDTreeBuilder(DDTreeBuilder):
    ...
    def __init__(
        ...
        *,
        ...
        if candidates[0] < self.block_size or candidates[-1] > self.tree_budget:
            raise ValueError("budget_candidates 必须位于 [block_size, tree_budget]")
        if int(initial_budget) not in candidates:
            raise ValueError("initial_budget 必须属于 budget_candidates")
        if hidden_size < 1 or learning_rate <= 0.0:
            raise ValueError("hidden_size 和 learning_rate 必须为正数")
        if not 0.0 <= gamma <= 1.0 or not 0.0 <= gae_lambda <= 1.0:
            raise ValueError("gamma 和 gae_lambda 必须位于 [0, 1]")
        if clip_range <= 0.0 or value_coefficient < 0.0 or entropy_coefficient < 0.0:
            raise ValueError("PPO loss 系数非法")
        if rollout_steps < 1 or update_epochs < 1 or minibatch_size < 1:
            raise ValueError("rollout/update/minibatch 参数必须为正整数")
        if max_grad_norm <= 0.0 or tree_build_cost_weight <= 0.0:
            raise ValueError("max_grad_norm 和 tree_build_cost_weight 必须为正数")
        if context_length_scale < 1:
    ...
    def _policy_inputs(
        ...
        if not available:
            ...
            return action_contexts, available_mask, state.astype(np.float32, copy=False)
    ...
    def _select_node_count(self, node_scores: np.ndarray) -> int:
        ...
        with torch.no_grad():
            ...
            action_tensor = (
                ...
                else distribution.logits.argmax()
            )
    ...
    def observe(
        ...
        *,
        ...
        if int(tree_nodes) != budget:
            raise ValueError("观测树节点数与最近 PPO 预算决策不一致")
        if not self.learning_enabled:
            return

        # ``draft_ms`` already contains tree construction.  Split it first, then
        # reweight tree construction so it is never counted twice accidentally.
        model_draft_ms = max(0.0, float(draft_ms) - float(tree_build_ms))
        weighted_latency_ms = (
            model_draft_ms
            + max(0.0, float(verify_ms))
            + self.tree_build_cost_weight * max(0.0, float(tree_build_ms))
        )
        reward = max(int(committed_tokens), 0) / max(weighted_latency_ms, 1e-6)
        ...
        previous_reward = self._reward_ewma.get(budget)
        self._reward_ewma[budget] = (
            reward if previous_reward is None else 0.9 * previous_reward + 0.1 * reward
        )
        previous_latency = self._latency_ewma.get(budget)
        self._latency_ewma[budget] = (
            weighted_latency_ms
            if previous_latency is None
            else 0.9 * previous_latency + 0.1 * weighted_latency_ms
        )
    ...
    def _update_policy(self) -> None:
        ...
        with torch.inference_mode(False), torch.enable_grad():
            ...
            actions = torch.tensor(
                [item.action for item in self._rollout], dtype=torch.long
            )
    ...
    def policy_state(self) -> dict[str, Any]:
        return {
            **self._configuration(),
            ...
            "selection_counts": {
                str(key): value for key, value in self._selection_counts.items()
            },
            "reward_ewma": {str(key): value for key, value in self._reward_ewma.items()},
            "latency_ewma": {
                str(key): value for key, value in self._latency_ewma.items()
            },
    ...
    def load_policy_state(self, state: dict[str, Any]) -> None:
        ...
        if mismatches:
            raise ValueError(f"PPO checkpoint 与当前配置不兼容: {mismatches}")
        ...
        if not isinstance(actor_state, dict) or not isinstance(critic_state, dict):
            raise ValueError("PPO checkpoint 缺少 actor/critic state")
        ...
        for raw in state.get("rollout", []):
            self._rollout.append(
                _Transition(
                    ...
                    available_mask=raw["available_mask"]
                    .cpu()
                    .numpy()
        ...
        self._selection_counts = {
            budget: int(counts.get(str(budget), 0)) for budget in self.budget_candidates
        }
        rewards = state.get("reward_ewma", {})
        self._reward_ewma = {
            budget: float(rewards[str(budget)])
            for budget in self.budget_candidates
            if str(budget) in rewards
        }
        latencies = state.get("latency_ewma", {})
        self._latency_ewma = {
            budget: float(latencies[str(budget)])
            for budget in self.budget_candidates
            if str(budget) in latencies
        }
        metrics = state.get("last_update_metrics", {})
        self._last_update_metrics = {
    ...
    def load_policy(self, path: str | Path) -> None:
        ...
        if not isinstance(state, dict):
            raise ValueError("PPO checkpoint 根节点必须是字典")
    ...
    def policy_diagnostics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "reward_ewma_tokens_per_ms": {
                str(key): value for key, value in self._reward_ewma.items()
            },
            "weighted_latency_ewma_ms": {
                str(key): value for key, value in self._latency_ewma.items()
            },
            "reward": {
                "formula": "committed_tokens / weighted_latency_ms",
                "weighted_latency_ms": (
                    "(draft_ms - tree_build_ms) + verify_ms + "
                    "tree_build_cost_weight * tree_build_ms"
                ),
                "tree_build_cost_weight": self.tree_build_cost_weight,
            },
