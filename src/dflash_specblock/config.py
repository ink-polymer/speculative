...
路径都相对于配置文件所在工程目录解析，便于将整个目录复制到 Atlas A2 服务器后直接运行。
"""
...
class ExperimentConfig:
    ...
    # 正式配置 fail-closed：没有 torch_npu/CANN 时直接报错，绝不静默切到 CPU。
    device: str = "npu:0"
    dtype: str = "bfloat16"
    block_size: int = 4
    max_blocks: int = 2
    ...
    def validate(self) -> None:
        ...
        supported_device = normalized_device in {"auto", "cpu"} or bool(
            re.fullmatch(r"npu(?::\d+)?", normalized_device)
        )
        if not supported_device:
            raise ValueError("device 仅支持 auto、cpu 或 npu:<id>")
