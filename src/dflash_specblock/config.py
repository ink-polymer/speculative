...
class ExperimentConfig:
    ...
    # ``ddtree_bandit`` 保留 contextual LinUCB 历史基线；``ddtree_ppo`` 是新的主 RL
    # 策略，用离散 PPO 根据当前 proposal、候选预算与上下文选择嵌套树前缀。
    ...
    ddtree_bandit_exploration_alpha: float = 0.15
    ddtree_bandit_ridge: float = 1.0
    ddtree_bandit_warmup_rounds_per_budget: int = 2
    ddtree_bandit_context_length_scale: int = 4096
    ddtree_bandit_train: bool = True
    ddtree_bandit_checkpoint: str | None = "checkpoints/ddtree_bandit_policy.json"
    ddtree_ppo_hidden_size: int = 64
    ddtree_ppo_learning_rate: float = 3e-4
    ddtree_ppo_gamma: float = 0.99
    ddtree_ppo_gae_lambda: float = 0.95
    ddtree_ppo_clip_range: float = 0.2
    ddtree_ppo_value_coefficient: float = 0.5
    ddtree_ppo_entropy_coefficient: float = 0.01
    ddtree_ppo_rollout_steps: int = 256
    ddtree_ppo_update_epochs: int = 4
    ddtree_ppo_minibatch_size: int = 64
    ddtree_ppo_max_grad_norm: float = 0.5
    ddtree_ppo_tree_build_cost_weight: float = 2.0
    ddtree_ppo_context_length_scale: int = 4096
    ddtree_ppo_train: bool = True
    ddtree_ppo_checkpoint: str | None = "checkpoints/ddtree_ppo_policy.pt"
    ...
    def validate(self) -> None:
        ...
        ddtree_modes = {"ddtree", "ddtree_adaptive", "ddtree_bandit", "ddtree_ppo"}
        if self.tree_mode not in {"specblock", *ddtree_modes}:
            raise ValueError(
                "tree_mode 仅支持 specblock、ddtree、ddtree_adaptive、"
                "ddtree_bandit 或 ddtree_ppo"
            )
        if not isinstance(self.ddtree_reserve_greedy_chain, bool):
            raise ValueError("ddtree_reserve_greedy_chain 必须是布尔值")
        if self.tree_mode in ddtree_modes:
        ...
        if self.tree_mode in {"ddtree_adaptive", "ddtree_bandit", "ddtree_ppo"}:
            ...
            if (
                self.tree_mode == "ddtree_adaptive"
                and self.ddtree_warmup_rounds_per_budget < 1
            ):
            ...
            if self.ddtree_reserve_greedy_chain:
                raise ValueError("自适应 DDTree 不与 reserve_greedy_chain 叠加")
            if self.use_cuda_graphs:
                raise ValueError(
                    "自适应 DDTree 需要可变 verify shape，不能使用固定最大形状 CUDA Graph"
                )
        if self.tree_mode == "ddtree_bandit":
            if self.ddtree_bandit_exploration_alpha < 0.0:
                raise ValueError("ddtree_bandit_exploration_alpha 不能为负数")
            if self.ddtree_bandit_ridge <= 0.0:
                raise ValueError("ddtree_bandit_ridge 必须为正数")
            if self.ddtree_bandit_warmup_rounds_per_budget < 0:
                raise ValueError(
                    "ddtree_bandit_warmup_rounds_per_budget 不能为负数"
                )
            if self.ddtree_bandit_context_length_scale < 1:
                raise ValueError("ddtree_bandit_context_length_scale 必须为正整数")
            if not isinstance(self.ddtree_bandit_train, bool):
                raise ValueError("ddtree_bandit_train 必须是布尔值")
            if not self.ddtree_bandit_train and not self.ddtree_bandit_checkpoint:
                raise ValueError("冻结 bandit 推理必须配置 ddtree_bandit_checkpoint")
        if self.tree_mode == "ddtree_ppo":
            if self.ddtree_ppo_hidden_size < 1:
                raise ValueError("ddtree_ppo_hidden_size 必须为正整数")
            if self.ddtree_ppo_learning_rate <= 0.0:
                raise ValueError("ddtree_ppo_learning_rate 必须为正数")
            if not 0.0 <= self.ddtree_ppo_gamma <= 1.0:
                raise ValueError("ddtree_ppo_gamma 必须位于 [0, 1]")
            if not 0.0 <= self.ddtree_ppo_gae_lambda <= 1.0:
                raise ValueError("ddtree_ppo_gae_lambda 必须位于 [0, 1]")
            if self.ddtree_ppo_clip_range <= 0.0:
                raise ValueError("ddtree_ppo_clip_range 必须为正数")
            if self.ddtree_ppo_value_coefficient < 0.0:
                raise ValueError("ddtree_ppo_value_coefficient 不能为负数")
            if self.ddtree_ppo_entropy_coefficient < 0.0:
                raise ValueError("ddtree_ppo_entropy_coefficient 不能为负数")
            if self.ddtree_ppo_rollout_steps < 1:
                raise ValueError("ddtree_ppo_rollout_steps 必须为正整数")
            if self.ddtree_ppo_update_epochs < 1:
                raise ValueError("ddtree_ppo_update_epochs 必须为正整数")
            if self.ddtree_ppo_minibatch_size < 1:
                raise ValueError("ddtree_ppo_minibatch_size 必须为正整数")
            if self.ddtree_ppo_max_grad_norm <= 0.0:
                raise ValueError("ddtree_ppo_max_grad_norm 必须为正数")
            if self.ddtree_ppo_tree_build_cost_weight <= 0.0:
                raise ValueError("ddtree_ppo_tree_build_cost_weight 必须为正数")
            if self.ddtree_ppo_context_length_scale < 1:
                raise ValueError("ddtree_ppo_context_length_scale 必须为正整数")
            if not isinstance(self.ddtree_ppo_train, bool):
                raise ValueError("ddtree_ppo_train 必须是布尔值")
            if not self.ddtree_ppo_train and not self.ddtree_ppo_checkpoint:
                raise ValueError("冻结 PPO 推理必须配置 ddtree_ppo_checkpoint")
    ...
    @property
    def ddtree_bandit_checkpoint_path(self) -> Path | None:
        if not self.ddtree_bandit_checkpoint:
            return None
        value = Path(self.ddtree_bandit_checkpoint).expanduser()
        return value.resolve() if value.is_absolute() else (self.project_root / value).resolve()

    @property
    def ddtree_ppo_checkpoint_path(self) -> Path | None:
        if not self.ddtree_ppo_checkpoint:
            return None
        value = Path(self.ddtree_ppo_checkpoint).expanduser()
        return value.resolve() if value.is_absolute() else (self.project_root / value).resolve()
