...
def test_default_plan_and_unchanged_full_alternative(monkeypatch, tmp_path):
    ...
    assert plan["variant_count"] == 22
    assert (plan["expected_records"], plan["expected_generations"]) == (51876, 57156)
    ...
    assert (full["expected_records"], full["expected_generations"]) == (240768, 246048)
    cfg["seeds"] = [777]
