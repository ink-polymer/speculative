...
def test_contextual_bandit_ddtree_config() -> None:
    project_root = Path(__file__).resolve().parents[1]
    train = ExperimentConfig.from_json(
        project_root / "configs" / "qwen3_4b_cuda_ddtree_bandit.json"
    )
    evaluation = ExperimentConfig.from_json(
        project_root / "configs" / "qwen3_4b_cuda_ddtree_bandit_eval.json"
    )

    assert train.tree_mode == "ddtree_bandit"
    assert train.ddtree_bandit_train is True
    assert train.ddtree_budget_candidates == (30, 45, 60, 80, 100, 128)
    assert train.use_cuda_graphs is False
    assert evaluation.ddtree_bandit_train is False
    assert evaluation.ddtree_bandit_checkpoint_path is not None


def test_frozen_bandit_requires_checkpoint_and_rejects_cuda_graph() -> None:
    missing_checkpoint = ExperimentConfig(
        tree_mode="ddtree_bandit",
        block_size=15,
        max_blocks=1,
        tree_budget=60,
        ddtree_budget_candidates=(30, 60),
        ddtree_initial_budget=60,
        ddtree_bandit_train=False,
        ddtree_bandit_checkpoint=None,
        rank_mode="heuristic",
        rank_checkpoint=None,
    )
    try:
        missing_checkpoint.validate()
    except ValueError as error:
        assert "checkpoint" in str(error)
    else:
        raise AssertionError("冻结 bandit 在无 checkpoint 时被接受")

    variable_shape_graph = ExperimentConfig(
        tree_mode="ddtree_bandit",
        block_size=15,
        max_blocks=1,
        tree_budget=60,
    ...
    try:
        variable_shape_graph.validate()
    except ValueError as error:
        assert "CUDA Graph" in str(error)
    else:
        raise AssertionError("bandit DDTree 错误接受了固定 shape CUDA Graph")
