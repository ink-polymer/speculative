...
OUTPUT = ROOT / "logs" / f"tree-gbv-v14-mid-budget-{os.environ['SLURM_JOB_ID']}.json"
...
def main():
        ...
        for budget in (5, 6, 7, 8, 9, 10):
            for virtual_k in (1, 2):
                methods.append(Variant(
                    name=f"prefix_mid_n{length}_b{budget}_k{virtual_k}",
    ...
    report = {
        "schema": 14, "complete": False,
        "purpose": "mid-budget tree GBV sweet-spot scan",
