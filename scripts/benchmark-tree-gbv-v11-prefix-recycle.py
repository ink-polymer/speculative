"""Same-process scan of root-diverse tree GBV with branch-recycled BV."""
...
OUTPUT = ROOT / "logs" / f"tree-gbv-v11-prefix-recycle-{os.environ['SLURM_JOB_ID']}.json"
...
def recycle_summary(results):
    rounds = [row for result in results for row in result["rounds"]]
    recycled = sum(row.get("recycled_corrections", 0) for row in rounds)
    return {
        "rounds": len(rounds),
        "recycled_corrections": recycled,
        "fraction_with_recycling": (
            sum(row.get("recycled_corrections", 0) > 0 for row in rounds) / len(rounds)
            if rounds else 0.0
        ),
    }
...
def main():
    ...
    methods = [
        ...
        Variant(
            name="tree_full_lf8_k4", method="tree_gbv_full", paths=4,
            length=14, temperature=1.0, draft_temperature=1.0,
            tree_budget=8, probability_dtype="float64",
        ),
    ]
    for length in (8, 11, 14):
        for leaves in (4, 6, 8):
            for virtual_k in (2, 4, 8):
                methods.append(Variant(
                    name=f"prefix_recycle_n{length}_lf{leaves}_k{virtual_k}",
                    method="tree_gbv_prefix_recycle", paths=virtual_k,
    ...
    report = {
        "schema": 11,
        "purpose": "root-diverse tree GBV with branch-recycled block verification",
    ...
    for prompt_index, prompt in enumerate(PROMPTS[:4]):
        ...
        report["validation"] = {
            name: {
                ...
                "recycling": recycle_summary(results),
            }
