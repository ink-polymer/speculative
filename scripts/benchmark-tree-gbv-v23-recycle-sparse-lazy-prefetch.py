"""Strict architecture-only screen for prefetch-optimized tree recycle GBV."""
...
RUN_SCHEMA = 23
RUN_TAG = os.environ.get("GBV_STRICT_TAG", "tree-gbv-v23-recycle-sparse-lazy-prefetch")
CANDIDATE_NAME = os.environ.get(
    "GBV_CANDIDATE_NAME", "hard_budget_recycle_sparse_lazy_prefetch"
)
CANDIDATE_METHOD = os.environ.get(
    "GBV_CANDIDATE_METHOD", "tree_gbv_budgeted_prefix_recycle_sparse_lazy_prefetch",
)
...
def main():
    ...
    report = {
        "schema": RUN_SCHEMA,
        "complete": False,
        "purpose": "strict architecture-only hard-budget prefetch recycle sparse-lazy screen",
        "fairness": fairness,
        "model": cfg["model"],
