...
OUTPUT = ROOT / "logs" / f"tree-gbv-v13-high-budget-{os.environ['SLURM_JOB_ID']}.json"
...
def main():
    ...
    for length in (8, 11, 14):
        for budget in (10, 12, 16):
            for virtual_k in (1, 2):
                methods.append(Variant(
                    name=f"prefix_hi_n{length}_b{budget}_k{virtual_k}",
    ...
    report = {
        "schema": 13, "complete": False,
        "purpose": "high-budget tree GBV with exact sparse block verification",
