...
def main() -> None:
    ...
    def run_action(ids: torch.Tensor, action: str, length: int):
        ...
        if kind == "ddtree":
            return ddtree_generate(**common, tree_budget=capacity)
        if kind == "dfs":
            return ddtree_generate(
                **common, tree_budget=capacity,
                depth_first_order=True, fast_timing=True,
            )
        if kind == "sparse":
            return ddtree_generate(
                **common, tree_budget=capacity, sparse_lm_head=True,
                depth_first_order=True, fast_timing=True,
            )
        if kind == "calibrated":
            return ddtree_generate(
        ...
        if kind == "rank":
            return ddtree_generate(
                **common, tree_budget=capacity, rank_head=rank_head,
        ...
        for index, row in enumerate(
            ...
            if not cfg.skip_comparators:
                ...
                fixed_metrics = metrics(
                    run_action(ids, f"ddtree:{cfg.fixed_tree_budget}", cfg.max_new_tokens)
                )
            torch.manual_seed(seed)
            torch.cuda.manual_seed_all(seed)
            selected_metrics = metrics(run_action(ids, action, cfg.max_new_tokens))
            policy.observe(
