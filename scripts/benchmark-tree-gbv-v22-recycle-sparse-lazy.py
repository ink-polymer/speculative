"""Strict architecture-only screen for hard-budget recycle GBV with sparse-lazy verifier."""
...
RUN_SCHEMA = 22
RUN_TAG = os.environ.get("GBV_STRICT_TAG", "tree-gbv-v22-recycle-sparse-lazy")
CANDIDATE_NAME = os.environ.get("GBV_CANDIDATE_NAME", "hard_budget_recycle_sparse_lazy")
CANDIDATE_METHOD = os.environ.get(
    "GBV_CANDIDATE_METHOD", "tree_gbv_budgeted_prefix_recycle_sparse_lazy",
)
...
def main():
    ...
    report = {
        "schema": RUN_SCHEMA,
        "complete": False,
        "purpose": "strict architecture-only hard-budget recycle sparse-lazy screen",
        "fairness": fairness,
        "model": cfg["model"],
