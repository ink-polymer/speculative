"""Distill a one-forward branch-aware slot mixer for DDTree construction."""
...
from gbv_experiments.slot_mixer import CausalSlotMixer
...
def prefix_coverage_kl(logits: torch.Tensor,
                       teacher_logits: torch.Tensor) -> torch.Tensor:
    teacher = torch.softmax(teacher_logits.float(), dim=-1)
    per_depth = F.kl_div(
        torch.log_softmax(logits.float(), dim=-1), teacher,
        reduction="none",
    ).sum(-1)
    weight = torch.arange(
...
def main() -> None:
    ...
    if (args.output.exists() or args.max_samples < 1
            ...
            or args.kernel_size < 2 or not 2 <= args.length <= 15):
        raise ValueError("Invalid or existing slot-mixer output/controls")
    ...
    mixer = CausalSlotMixer(
        int(engine.draft.config.hidden_size), args.rank, args.kernel_size,
    ...
    embedding = engine.target.get_output_embeddings()
    ...
    def example(index: int, seed: int):
        ...
        return (
            ...
            draft_logits[:, :args.length].clone(), teacher_logits.clone(),
        )
    ...
    for epoch in range(args.epochs):
        ...
        for step, index in enumerate(shuffled, 1):
            ...
            corrected_logits = embedding(mixer(hidden))
            loss = prefix_coverage_kl(corrected_logits, teacher_logits)
            with torch.no_grad():
                baseline = prefix_coverage_kl(draft_logits, teacher_logits)
            loss.backward()
    ...
    with torch.inference_mode():
        for index in holdout:
            ...
            if batch is None:
                continue
            hidden, draft_logits, teacher_logits = batch
            held += float(prefix_coverage_kl(
                embedding(mixer(hidden)), teacher_logits,
            ))
            held_baseline += float(prefix_coverage_kl(
                draft_logits, teacher_logits,
            ))
            held_count += 1
    if updates < 1 or held_count < 1:
        raise RuntimeError("Slot mixer received no usable examples")
    metadata = {
        "architecture": "parallel_causal_slot_mixer_v1",
        "hidden_size": mixer.hidden_size, "rank": mixer.rank,
        "kernel_size": mixer.kernel_size, "updates": updates,
