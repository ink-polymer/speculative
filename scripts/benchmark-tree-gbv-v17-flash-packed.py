...
OUTPUT = ROOT / "logs" / f"tree-gbv-v17-flash-packed-{os.environ['SLURM_JOB_ID']}.json"
...
def run_generate(engine, *args, **kwargs):
    variant = args[1]
    previous = engine.target.config._attn_implementation
    engine.target.config._attn_implementation = (
        "sdpa" if variant.method == "ddtree" else "flash_attention_2"
    )
    try:
        return engine.generate(*args, **kwargs)
    finally:
        engine.target.config._attn_implementation = previous
...
def main():
    ...
    for length in (8, 11, 14):
        for budget in (32,):
            for virtual_k in (2, 3, 4):
                methods.append(Variant(
                    name=f"sampled_packed_n{length}_b{budget}_k{virtual_k}",
                    method="tree_gbv_packed", paths=virtual_k,
                    length=length, temperature=1.0, draft_temperature=1.0,
                    tree_budget=budget, probability_dtype="float64",
                ))
    report = {
        "schema": 17, "complete": False,
        "purpose": "FlashAttention-2 sampled-path packed GBV architecture",
    ...
    for index, variant in enumerate(methods):
        run_generate(engine, warmup, variant, 24, [], seed=index + 1, profile=False)
        ...
        for seed in (1709, 2903):
            ...
            for variant in order:
                collected[variant.name].append(
                    run_generate(engine, ids, variant, 96, [], seed=seed, profile=True)
                )
