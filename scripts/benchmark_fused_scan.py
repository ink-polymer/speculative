"""Gate a same-real-DDTree, one-kernel scan verifier against official DDTree."""
...
def summarize(rows: list[dict], bootstrap_samples: int = 10000) -> dict:
    clusters = {}
    for row in rows:
    ...
    for state, values in sorted(clusters.items()):
        ratios = [
            values["ddtree", repeat] / values["fused_scan", repeat]
    ...
    return {
        "metric": "same_real_tree_ddtree_verifier_speedup",
        "speedup": speedup,
...
def run(config: Path, output: Path, device: str, states_per_prompt: int,
    ...
    with output_lock(output.parent):
        ...
        report = {
            "kind": "same_real_ddtree_fused_scan_gate",
