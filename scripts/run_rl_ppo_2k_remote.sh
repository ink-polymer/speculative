...
TRAIN_CONFIG="${TRAIN_CONFIG:-configs/qwen3_4b_cuda_ddtree_ppo.json}"
EVAL_CONFIG="${EVAL_CONFIG:-configs/qwen3_4b_cuda_ddtree_ppo_eval.json}"
...
TRAIN_OUTPUT="${TRAIN_OUTPUT:-outputs/rl_ppo_2k/train_policy_566_128.jsonl}"
EVAL_OUTPUT="${EVAL_OUTPUT:-outputs/rl_ppo_2k/eval_2k_2048.jsonl}"
TRAIN_MAX_NEW_TOKENS="${TRAIN_MAX_NEW_TOKENS:-128}"
EVAL_MAX_NEW_TOKENS="${EVAL_MAX_NEW_TOKENS:-2048}"
STATUS_FILE="${STATUS_FILE:-outputs/rl_ppo_2k/run_status.txt}"
PPO_CHECKPOINT="checkpoints/ddtree_ppo_policy.pt"

mkdir -p "$(dirname "${TRAIN_OUTPUT}")" "$(dirname "${EVAL_OUTPUT}")" \
    "$(dirname "${PPO_CHECKPOINT}")" logs
...
sha256sum "${PPO_CHECKPOINT}" "${TRAIN_OUTPUT}" "${EVAL_OUTPUT}" \
    > outputs/rl_ppo_2k/SHA256SUMS
