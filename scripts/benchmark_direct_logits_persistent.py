"""Development-only gate for direct-logits persistent DDTree verification.
...
def variants() -> list[Variant]:
    ...
    values = [
        ...
        replace(base, name="direct_logits_fused_scan",
                method="ddtree_direct_logits_fused_scan"),
    ]
    for value in values:
        value.validate()
    return values
...
def run(config: Path, output: Path, device: str, tokens: int,
    ...
    fairness = {
        ...
        for name in ("fused_scan", "direct_logits_fused_scan")
    }
    ...
    with output_lock(output):
        ...
        manifest = {
            "kind": "development_direct_logits_persistent_gate",
            ...
            "success_rule": (
                "the direct-logits candidate must have DDTree CI low > 1.03, "
                "fused-scan CI low > 1.01, DFlash CI low > 1, and visit fewer "
                "probability rows; DDTree/DFlash CI low must exceed 1"
            ),
            "source_sha256": {
        ...
        comparisons = {
            "candidate_vs_ddtree": compare(
                rows, "direct_logits_fused_scan", "ddtree",
            ),
            "candidate_vs_fused_scan": compare(
                rows, "direct_logits_fused_scan", "fused_scan",
            ),
            "candidate_vs_dflash": compare(
                rows, "direct_logits_fused_scan", "dflash",
            ),
