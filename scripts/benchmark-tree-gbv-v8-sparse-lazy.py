"""Same-process A/B of eager BV and sparse-support lazy BV."""
...
OUTPUT = ROOT / "logs" / f"tree-gbv-v8-sparse-lazy-{os.environ['SLURM_JOB_ID']}.json"
...
def main():
    ...
    methods = [
        Variant(name="tree_sparse_eager", method="tree_gbv_full", **common),
        Variant(
            name="tree_sparse_support_lazy",
            method="tree_gbv_full_sparse_lazy",
            **common,
        ),
        Variant(
            name="ddtree",
    ...
    report = {
        "schema": 8,
        "purpose": "same-process eager-versus-sparse-support-lazy BV A/B",
        ...
        "distribution_contract": {
            ...
            "validation": "exhaustive output-law enumeration; duplicate-support residual identity; full suite",
        },
        "implementation": {
            "eager_dense_residual_rows": 15,
            "lazy_dense_residual_rows": 1,
            "proposal_support_width": 6,
        },
    ...
    eager = report["validation"]["tree_sparse_eager"]["summary"]
    lazy = report["validation"]["tree_sparse_support_lazy"]["summary"]
