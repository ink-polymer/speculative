"""Narrow same-process validation of sparse prefix-tree GBV recycling."""
...
OUTPUT = ROOT / "logs" / f"tree-gbv-v12-sparse-recycle-{os.environ['SLURM_JOB_ID']}.json"
...
def main():
    ...
    for length, leaves, virtual_k in (
        (11, 4, 2), (11, 6, 2), (14, 4, 2), (14, 6, 2),
        (14, 8, 2), (14, 4, 4), (14, 6, 4),
    ):
        methods.append(Variant(
            name=f"sparse_prefix_n{length}_lf{leaves}_k{virtual_k}",
    ...
    report = {
        "schema": 12,
        "purpose": "sparse block verification on a root-diverse GBV tree",
    ...
    for name, value in report["validation"].items():
        ...
        ranking.append({
            ...
            "stage_cuda_ms": result["stage_cuda_ms"],
        })
    ranking.sort(key=lambda row: row["decode_tokens_per_second"], reverse=True)
    report["ranking"] = ranking
    report["complete"] = True
    save(report)
    print(json.dumps({"ddtree_tps": ddtree_tps, "top": ranking}, indent=2), flush=True)
