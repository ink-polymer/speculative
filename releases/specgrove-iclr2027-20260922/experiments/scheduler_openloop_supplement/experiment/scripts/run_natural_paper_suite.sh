...
study=/root/dp-paper-natural-once-20260917
results=/root/dp-paper-natural-once-results-20260917
previous=/root/dp-paper-natural-results-20260917
...
for smoke_phase in native_c1 ablation correctness; do
  "$runtime" scripts/run_natural_paper_phase.py --model "$model" --phase "$smoke_phase" --smoke \
    ...
    --reuse-directory "$previous/$model/nonformal-smoke-$smoke_phase" \
    >> "$results/$model/smoke.log" 2>&1 || exit 4
done
# Dedicated long-output sweep cancelled by user.
for phase in main128 quality correctness native_c1 budget ablation heterogeneous; do
  "$runtime" scripts/run_natural_paper_phase.py --model "$model" --phase "$phase" \
    ...
    --reuse-directory "$previous/$model/$phase" \
