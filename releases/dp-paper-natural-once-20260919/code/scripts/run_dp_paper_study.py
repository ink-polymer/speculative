"""Resumable real-GPU batch study, not a native streaming-serving benchmark."""
import argparse
...
def source_hashes():
    paths=list((ROOT/'src/gbv_experiments').glob('*.py'))+[Path(__file__),ROOT/'scripts/paper_dp_allocators.py']
...
def methods_for(case):
    if case['phase']=='ablation':
        return ['dp','equal','greedy','random','no_weight','constant_cost','linear_cost','no_tier_fit','no_active_gate','age_admission']
    return ['ar','dflash','ddtree','dp']
...
def main():
    ...
    contract=cases_for(args.phase);hashes=source_hashes()
    write(args.output/'contract.json',{'model':args.model,'phase':args.phase,'cases':contract,'source_hashes':hashes,
        'methods':methods_for(contract[0]),'kind':'preregistered_common_framework_real_GPU_batch_study',
    ...
    cache={};ordinal=0;warmed=set();counts={'completed_groups':0,'skipped_groups':0,'failed_groups':0}
