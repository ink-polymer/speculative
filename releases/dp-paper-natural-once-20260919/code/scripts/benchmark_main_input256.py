#!/usr/bin/env python3
"""Full fixed-256-input natural-task main matrix, separate from synthetic pilots."""
import argparse
import hashlib
import json
...
from benchmark_continuous_tree_block_e2e import summarize
...
from paper_input256 import select_input256
...
def main():
    ...
    for dataset in datasets:
        cpu_prompts[dataset], selections[dataset] = select_input256(
            tokenizer, cfg, args.data_dir / f'{dataset}.jsonl', args.requests)
    ...
    contract = {'model': cfg, 'source_hashes': hashes, 'temperature': 1, 'input_tokens': 256,
        ...
        'kind': 'fixed_input256_natural_task_main', 'protocol_version': 1,
        'native_official_dflash': False, 'strict_BF16_AR_certified': False,
        'input_control': 'Original complete tasks <=256 tokens, attended newline-prefix fill to256; no task truncation',
        ...
        'quality': 'Generation only; modified prompts, no scores or strict exactness certification'}
    ...
    for case in pending:
        ...
        request_seeds = [case['seed'] + 104729 * index for index in range(first, first + c)]
        if c not in warmed:
