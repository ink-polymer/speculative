"""Summarize amended single-wave study, explicitly showing unsupported skips."""
import argparse
...
import random
import statistics
from paper_single_wave_policy import methods_for_case, exclusion_records
...
def main():
    ...
    args=p.parse_args();groups=defaultdict(lambda:defaultdict(list));quality=[];statuses=[];expected=defaultdict(int);policy_exclusions=[];quality_methods={}
    for phase in ['pilot','main','budget','ablation','temperature0','long_output','context','quality_math']:
        for model in ['qwen3_4b','qwen3_8b']:
            ...
            if contract.exists():
                registered=json.loads(contract.read_text())
                if 'single_wave_policy' not in registered:raise RuntimeError('Use this summary only for amended single-wave results')
                for item in registered['single_wave_policy']['cases']:
                    if item['excluded_methods']:policy_exclusions.append({'phase':phase,'model':model,**item})
                for case in registered['cases']:
                    if phase=='quality_math':
                        quality_methods[(model,case['dataset'])]=set(methods_for_case(case))
    ...
    root=args.directory
    skip_header=['','## Unsupported configurations: skipped by policy','',
        '|Phase|Model|Dataset|T|R|C|Seed|Output cap|Method|Single-wave capacity|Reason|',
        '|---|---|---|---:|---:|---:|---:|---:|---|---:|---|']
    table.extend(skip_header)
    split_skips={}
    for item in policy_exclusions:
        case=item['case']
        for excluded in item['excluded_methods']:
            line='|'+ '|'.join(map(str,[item['phase'],item['model'],case['dataset'],case['temperature'],
                case['budget'],case['concurrency'],case['seed'],case['max_new_tokens'],excluded['method'],
                excluded['single_wave_capacity'],excluded['reason']]))+'|'
            table.append(line)
            split_skips.setdefault((case['temperature'],case['budget']),list(skip_header)).append(line)
    for key,lines in split_skips.items():
        separate_tables.setdefault(key,list(header)).extend(lines)
    (root/'RESULTS.md').write_text('\n'.join(table)+'\n')
    for (temperature,budget),lines in separate_tables.items():
        (root/f'RESULTS_T{temperature}_R{budget}.md').write_text('\n'.join(lines)+'\n')
    (root/'RESULTS.json').write_text(json.dumps({'statuses':statuses,'cells':machine,'policy_exclusions':policy_exclusions,'speedup_reference':'matched autoregressive',
    ...
    for model in ['qwen3_4b','qwen3_8b']:
        for dataset in ['gsm8k','math500']:
            ...
            if not all(set(v)==quality_methods[(model,dataset)] and all(len(x)==5 for x in v.values()) for v in selected):
            ...
            reports.append({'model':model,'dataset':dataset,'independent_prompt_clusters':len(selected),
                'eligible_methods':sorted(quality_methods[(model,dataset)]),
                'excluded_methods':sorted({'ar','dflash','ddtree','dp'}-quality_methods[(model,dataset)]),
