"""Amended single-wave study. Original frozen driver/core and results are preserved."""
import argparse
...
import torch
import run_dp_paper_study as legacy_driver
...
def source_hashes():
    paths=list((ROOT/'src/gbv_experiments').glob('*.py'))+[Path(__file__),ROOT/'scripts/paper_dp_allocators.py',ROOT/'scripts/paper_single_wave_policy.py',ROOT/'scripts/run_dp_paper_study.py']
...
def main():
    ...
    write(args.output/'contract.json',{'model':args.model,'phase':args.phase,'cases':contract,'source_hashes':hashes,
        ...
        'kind':'amended_single_target_wave_common_framework_real_GPU_batch_study',
    ...
    for case in contract:
        ...
        for group,first in enumerate(range(offset,offset+n,c)):
            ...
            if args.reuse_directory and (args.reuse_directory/'groups'/path.name).exists():
                ...
                imported['measurement_source_hashes']=previous['source_hashes']
