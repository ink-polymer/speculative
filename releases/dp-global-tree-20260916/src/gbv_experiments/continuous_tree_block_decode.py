...
def _propose_many(
    ...
    if reuse_request_draft_cache:
        if variant.method != "ddtree":
            raise ValueError("Packed persistent Draft cache currently supports DDTree")
...
def generate_continuous_tree_blocks(
    ...
    if (
        ...
        or (reuse_request_draft_cache and variant.method != "ddtree")
    ...
    if batched_prefill:
        prompt_lengths = tuple(int(state.input_ids.shape[1]) for state in states)
        maximum_prompt = max(prompt_lengths)
        ids = states[0].input_ids.new_zeros((len(states), maximum_prompt))
        positions = torch.zeros_like(ids)
        attention = torch.zeros_like(ids, dtype=torch.bool)
        for slot, (state, length) in enumerate(zip(states, prompt_lengths)):
        ...
        last_hidden = output.last_hidden_state[
            torch.arange(len(states), device=engine.device), last_rows
        ]
        ...
        for state in states:
            ...
            output = engine.target_forward(
                state.input_ids, state.target_cache, hidden=True, last_only=True,
            )
            anchor = (
