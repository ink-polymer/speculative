...
def ddtree_generate(
    ...
    if target_verifier is not None:
        if temperature != 0.0:
            raise ValueError("graphed DDTree verification requires temperature=0")
        if int(target_verifier.max_verify_len) < max_tree_nodes:
            raise ValueError(
                "target verifier shape is smaller than the configured DDTree"
            )
    ...
    past_key_values_target = (
        ...
        else target_verifier.static_cache
    )
    ...
    if target_verifier is None:
        ...
        anchor_token, target_hidden, prefill_ms = target_verifier.prefill(input_ids)
        output_ids[0, num_input_tokens] = int(anchor_token)
        time_to_first_token = float(prefill_ms) / 1000.0
    ...
    while start < max_length:
        ...
        if target_verifier is None:
            ...
            graph_tree = DraftTree()
            for node_index, (token_id, parent_index) in enumerate(
                zip(node_token_ids.tolist(), parents[1:]), start=1
            ):
                graph_tree.add_node(
                    token_id=int(token_id),
                    parent=int(parent_index) - 1,
                    cumulative_log_probability=0.0,
                    block_index=0,
                    slot_index=int(node_depths[node_index - 1]) - 1,
                    rank_bucket=0,
                )
            graph_tree.preset_ancestor_mask(visibility_cpu[1:, 1:])
            tree_compile_elapsed = 0.0

        verify_stage_start = stage_clock()
        if target_verifier is not None:
            graph_result = target_verifier.verify(
                anchor_token_id=int(root_token[0, 0]),
                tree=graph_tree,
                cache=past_key_values_target,
                past_length=start,
            )
            accepted_indices = [0] + [
                int(index) + 1 for index in graph_result.path.node_indices
            ]
            next_token = int(graph_result.path.bonus_token_id)
        elif sparse_lm_head:
            output = target.model(
        ...
        if target_verifier is not None:
            accepted_node_indices = torch.tensor(
                [index - 1 for index in accepted_indices[1:]],
                dtype=torch.long,
            )
            accepted_tokens = torch.cat(
                [
                    root_token,
                    node_token_ids.index_select(0, accepted_node_indices)
                    .to(device=root_token.device)
                    .unsqueeze(0),
                ],
                dim=1,
            )
            accepted_index_tensor = None
        elif contiguous_accept:
        ...
        if target_verifier is not None:
            target_hidden = graph_result.target_context
        else:
